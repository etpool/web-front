<template>
  <div :class="pageView">
    ...
    <div v-if="showFooter" class="footer">
      ...
      <!--<div style="background: #192330;color: #828ea1;padding: 40px 80px;">-->
      <!--  <div>-->
      <!--    {{$t("common.footer1")}}-->
      <!--  </div>-->
      ...
      <!--  </div>-->
      <!--</div>-->
      <div style="color: #828ea1;padding: 0 80px;">
        {{$t("common.footer3")}}
      </div>
    </div>
    <template>
...
export default {
  ...
  watch: {
    ...
    $route(to, from) {
      ...
      if(to.path.length >= 5 &&  to.path.substr(0,5) == "/mlab"){
...
　　let flag = navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i)
    ...
    downloadUrlList(){
      this.$http
          .post(this.host + "/uc/ancillary/system/app/version/list", {})
          .then(response => {
            var resp = response.body || {}
            var data = resp.data || []
            for (let item of data) {
              if(item.platform == 0) {
                this.androidDownloadUrl = item.downloadUrl
              } else if(item.platform == 1) {
                this.iosDownloadUrl = item.downloadUrl
              }
            }
          });
    },
    ...
    .layout {
      ...
      .layout-ceiling {
        ...
        .layout-logo{
          width: 248px;
          height: 50px;
          // background: url(./assets/images/logo_1.png) no-repeat left 8px;
