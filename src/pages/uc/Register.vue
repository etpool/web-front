          ...
          <Input type="text" v-model="formInline.user" :placeholder="key" v-if="changeActive==0">
            <Select v-model="country" slot="prepend" style="width: 65px;border-bottom: 1px solid #27313e;">
              <Option v-for="(area,index) in areas" :key="index"  :value="area.zhName" :label="'+' + area.areaCode">
                ...
                <span v-if="lang=='简体中文'" style="margin-left:10px;color:#ccc">{{area.zhName}}</span>
                <span v-else style="margin-left:10px;color:#ccc">{{area.enName}}</span>
              </Option>
            </Select>
          </Input>
    ...
    getAreas() {
      ...
      this.$http.post(this.host + this.api.common.area, params).then(response => {
        ...
        if(resp.data.length > 0){
          ...
          this.language = this.areas[0].language;
